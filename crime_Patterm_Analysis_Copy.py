...
def chloropleth_maps(df, geojson_data, mean_lat, mean_lon):
    ...
    options = ['Choose an option', 'Crime Incidents', 'Total Victim Count', 'Total Accused Count']
    selected_stat = st.selectbox('Select Crime Statistic', options)
    ...
    elif selected_stat == 'Total Accused Count':
        fig = px.choropleth_mapbox(district_stats,
                                   geojson=geojson_data,
                                   locations='District_Name',
                                   featureidkey="properties.district",
    ...
    elif selected_stat == 'Choose an option':
        st.write("Please select a crime statistic from the dropdown menu.")

    else:
        st.write("Invalid selection. Please choose a valid option.")
